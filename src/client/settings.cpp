...
settings_class::settings_class(bool file_exists, bool *dev){
    ...
    if (file_exists){
        ...
        controls[up].input.keyInput = sf::Keyboard::W;

        controls[down].iType = inputType::KEYBOARD;
        controls[down].input.keyInput = sf::Keyboard::S;

        controls[left].iType = inputType::KEYBOARD;
        controls[left].input.keyInput = sf::Keyboard::A;

        controls[right].iType = inputType::KEYBOARD;
        controls[right].input.keyInput = sf::Keyboard::D;

        controls[prevItem].iType = inputType::KEYBOARD;
        controls[prevItem].input.keyInput = sf::Keyboard::Q;

        controls[nextItem].iType = inputType::KEYBOARD;
        controls[nextItem].input.keyInput = sf::Keyboard::E;
        ...
        controls[special].input.keyInput = sf::Keyboard::LShift;

        controls[useItem].iType = inputType::KEYBOARD;
        controls[useItem].input.keyInput = sf::Keyboard::G;

        controls[jump].iType = inputType::KEYBOARD;
        controls[jump].input.keyInput = sf::Keyboard::Space;
    ...
    if (*p_dev){
        ...
        for (short i = 0; i < n_keyInputOptions; i++){
            std::cout << "\t\t" << controls[i].iType;
            if (controls[i].iType == inputType::KEYBOARD){
                std::cout << "\t" << controls[i].input.keyInput << std::endl;
            }
            else if (controls[i].iType == inputType::MOUSE_BUTTON){
                std::cout << "\t" << controls[i].input.mouseInput << std::endl;
            }
        }
        std::cout << std::endl;
    }
